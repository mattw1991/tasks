...
#include <string>
#include <cstdlib>
#include <cassert>  //added by Matt
#include <regex>    //added by Matt
...
private:
    ...
    Type findType (std::string input) const
    {
        ...
        if (find (input, "/")) return Type::divide;     //added by Matt
        return Type::unknown;
    }
    ...
    std::optional <double> findLHS (std::string input) const
    {
        ...
        if (auto result = findAndExtractLHS (input, "/"))   //added by Matt
            return result;                                  //added by Matt
    ...
    std::optional <double> findRHS (std::string input) const
    {
        ...
        if (auto result = findAndExtractRHS (input, "/"))   //added by Matt
            return result;                                  //added by Matt
    ...
    double calculate (Tokeniser::TokenList tokens) const
    {
        switch (tokens.type)
        {
            ...
            case Tokeniser::Type::multiply:
                return tokens.lhs * tokens.rhs;
            case Tokeniser::Type::divide:           //added by Matt
                return tokens.lhs / tokens.rhs;     //added by Matt
            default:
                break;
        }
...
public:
    ...
    InputRequest requestInput () const
    {
        ...
        input = std::regex_replace(input, std::regex("pi"), "3.14159");   //added by Matt
...
void test ()
{
    ...
    ResultChecker::check (Calculator ().calculate ({ 3, 5.6, Tokeniser::Type::subtract }), -2.6);   //changed by Matt to '-2.6'
    ResultChecker::check (Calculator ().calculate ({ 3.141, 5, Tokeniser::Type::multiply }), 15.705);  //added by Matt
